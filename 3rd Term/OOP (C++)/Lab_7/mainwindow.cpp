...
void MainWindow::on_Add_from_file_button_clicked()
{
    ...
    if (ui->lineEdit_filepath->text().endsWith(".csv"))
    {
        ...
        auto temp = Reader->readAll();
        Collection.add_cars(std::get<0>(temp));

        if (std::get<1>(temp).size() == 1)
        {
            errorsPresence = true;
            ui->lineEdit_filepath->setText(QString("Skipped corrupted line: %1").arg(std::get<1>(temp)[0]));
        }
        else if (std::get<1>(temp).size() > 1)
        {
            errorsPresence = true;
            QString message = "Skipped corrupted lines: ";
            std::vector<int> CL = std::get<1>(temp);
            for (unsigned int i = 0; i < CL.size(); i++)
            {
                message += QString::number(CL[i]);
                if (i < CL.size() - 1) message += ", ";
            }
            ui->lineEdit_filepath->setText(message);
        }
        delete Reader;
    }
    ...
    if (Collection.Cars.size() != prevSize && errorsPresence)
    {
        updateTableView();
    }
    else if (Collection.Cars.size() != prevSize && !errorsPresence)
    {
